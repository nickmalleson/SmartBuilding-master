...
def managed_space_nrows(scraper_instance,
    ...
    Parameters
    ----------
    scraper_instance: object
        Class instance generated with 'Scraper()'. Is required for the 
        'username', 'password', and 'scraper_instance.managed_space_info' 
        variables.
    managed_space_numbers: int/list of ints
        Single number or list. Use: 
        choose_by_number(scraper_instance.managed_space_info, 'managed space')        
    ...
    all_managed_spaces_numbers = \
        list(range(1, len(scraper_instance.managed_space_info)+1))
    ...
    for num, i in enumerate(managed_space_numbers, start=0):
        space = scraper_instance.managed_space_info[i-1]
        urlstr = str('https://console.beringar.co.uk/api/beta/managedspace/'
                     'spacelocation/{}/after/{}'
                     .format(space['id'], timestamp_epoch_millisec))
        response = r.get(urlstr, auth=(scraper_instance.username,
                                       scraper_instance.password))
        responsecheck = response.status_code
...
def sensor_reading_after(scraper_instance,
                         sensor_numbers=None,
                         timestamp_epoch_millisec=1000):
    ...
    scraper_instance: object
        Class instance generated with 'Scraper()'. Is required for the 
        'username', 'password', and 'sensor_location_info' variables.
    sensor_numbers: int/list of ints
        Single number or list. Use: 
        choose_by_number(scraper_instance.sensor_location_info, 'sensor')        
        to get corresponding numbers.
    timestamp_epoch_millisec: int
    ...
    all_sensor_numbers = \
        list(range(1, len(scraper_instance.sensor_location_info)+1))
    ...
    for num, i in enumerate(sensor_numbers, start=0):
        sensor = scraper_instance.sensor_location_info[i-1]
...
def sensor_reading_last(scraper_instance,
                        sensor_numbers=None,
                        timestamp_epoch_millisec=1000):
    ''' Get last ‘max_rows’ of sensor readings for a given sensor location.
...
def plot_managed_spaces(scraper_instance, managed_spaces=None,
    ...
    if managed_space_nrows_data is None and \
       managed_spaces is None:
        managed_space_nrows_data, managed_spaces = \
            managed_space_nrows(scraper_instance)
    elif managed_space_nrows_data is None:
        managed_space_nrows_data, _ = \
            managed_space_nrows(scraper_instance,
                                managed_space_numbers=managed_spaces)
    else:
    ...
    for data, space_number in zip(managed_space_nrows_data,
        ...
        if not data:
            print('Managed space number {}: {}. NO DATA RETURNED'
                  .format(space_number,
                          scraper_instance.
                          managed_space_info[space_number-1]['name']))
            continue
        else:
            legendlabels.append(
                scraper_instance.managed_space_info[space_number-1]['name'])
            print('Managed space number {}: {}. Data Available. '
                  'Plotting chart.'
                  .format(space_number,
                          scraper_instance.managed_space_info[space_number-1]
                          ['name']))
...
def plot_sensor_reading_after(scraper_instance, sensor_numbers=None,
    ...
    sensor_numbers: int/list of ints
        Single number or list. Use: 
        choose_by_number(scraper_instance.get_sensor_location_info, 'sensor')        
        to get corresponding numbers.
    ...
    if sensor_reading_after_data is None and sensor_numbers is None:
        sensor_reading_after_data, sensor_numbers = \
            sensor_reading_after(scraper_instance)
    elif sensor_reading_after_data is None:
        sensor_reading_after_data, _ = \
            sensor_reading_after(
                scraper_instance, sensor_numbers=sensor_numbers)
        ...
        if not data:
            print('Sensor number {}: {}. NO DATA RETURNED'
                  .format(sensor_number,
                          scraper_instance.
                          sensor_location_info[sensor_number-1]['name']))
            continue
        else:
...
def plot_sensor_reading_last(scraper_instance, sensor_numbers=None,
    ...
    if sensor_reading_last_data is None and sensor_numbers is None:
        sensor_reading_last_data, sensor_numbers = \
            sensor_reading_last(scraper_instance)
